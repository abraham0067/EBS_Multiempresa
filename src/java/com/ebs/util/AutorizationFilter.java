...
public class AutorizationFilter implements Filter {
    ...
    public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain) throws ServletException, IOException {
        ...
        String loginURL = request.getContextPath() + "/login/login.xhtml";
        String trialPage = request.getContextPath() + "/login/trial.xhtml";
        String homePage = request.getContextPath() + "/main/principal.xhtml";
        String regURL = request.getContextPath() + "/login/autoRegistro.xhtml";
        String recURL = request.getContextPath() + "/recovery/passwordRecovery.xhtml";
        String activationURL = request.getContextPath() + "/activation.xhtml";

        boolean loggedIn = (session != null) && (session.getAttribute("macceso") != null);
        boolean loginRequest = request.getRequestURI().equals(loginURL);
        boolean registerRequest = request.getRequestURI().equals(regURL);
        boolean recoverRequest = request.getRequestURI().equals(recURL);
        boolean activationRequest = request.getRequestURI().equals(activationURL);
        ...
        if (loggedIn && !resourceRequest && !loginRequest) {
            canAcces = canUserAcces((MAcceso) session.getAttribute("macceso"), getLongPage((String) request.getRequestURL().toString()));
            if (!canAcces) {
                response.sendRedirect(homePage); // So, just perform standard synchronous redirect.
            }
        }
        if (loggedIn || loginRequest || resourceRequest || registerRequest || activationRequest || recoverRequest && canAcces) {
